...
public class TG_5803_DirectOrderDeny extends TestBase {
	...
	public void loader() throws IOException {
		log = LogManager.getLogger(TG_5803_DirectOrderDeny.class.getName());
	...
	@Test(dataProvider = "TG_5803_automate_direct_order_deny_order")
	public void VerifyUserRedirectToNewOrderFormViaDirectOrder(String url, String usernameHiring, String passwordHiring,
			...
			String eSignature, String usernameStaffing2, String passwordStaffing2) throws InterruptedException {
		test = extent.createTest("Verify Confirm Job Order Details");
		test.log(Status.INFO,"Started==Verifying Confirm Job Order Details");
		
		test.log(Status.INFO,"Fetching the URL");
		driver.get(url);
		test.log(Status.PASS, "URL fetched successfully");
...
//		test.log(Status.PASS,"Clicked on Clear Filter");
		...
		Thread.sleep(5000);
		test.log(Status.PASS, "Clicking on Deny Button");
		block.clickDenyButton();
		test.log(Status.PASS, "Deny Button clicked successfully");
		...
		test.log(Status.INFO,"Entering Username");
		login.enterUsername(usernameStaffing2);
		test.log(Status.PASS,"Username Entered Successfully");
		
		test.log(Status.INFO,"Entering Password");
		login.enterPassword(passwordStaffing2);
		test.log(Status.PASS,"Password Entered Successfully");
		...
		Thread.sleep(2000);
		test.log(Status.INFO,"Verifying Job order page");
		Assert.assertEquals(jo.verifyJobOrderPage(), true, "Job order page title not displayed");
		test.log(Status.PASS, "Job order page verified successfully");
		Thread.sleep(10000);

		test.log(Status.INFO, "Selecting Particular Joborder on the listing");
		System.out.println(jobOrderId);
		block.selectParticularJobOrderFromListingStaff(jobOrderId);
		test.log(Status.PASS, "Particular Joborder on the listing selected successfully");
		Thread.sleep(5000);

	}

	@DataProvider(name = "TG_5803_automate_direct_order_deny_order")
	public Object[][] data() throws IOException, ParserConfigurationException, SAXException {
		XmlReader reader = new XmlReader();
		return reader.testData("TG_5803_automate_direct_order_deny_order");
	}
