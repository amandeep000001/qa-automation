...
import com.sf.pages.TimesheetDenyPage;
import com.sf.utility.Functions;
import com.sf.utility.XmlReader;

public class TG_4423_TimesheetDeny extends TestBase {
	LoginPage login;
	JobOrderPage order;
	Functions functions;
	...
	public void loader() throws IOException {
		log = LogManager.getLogger(TG_4423_TimesheetDeny.class.getName());
		...
		deny = new TimesheetDenyPage(driver, log);
		job = new JobOrderPage(driver, log);
		delete = new DeleteJobOrderPage(driver, log);
	}

	@Test(dataProvider = "TG_TimesheetDeny")
	public void Tag_EndToEndFlowNewOrderHiring(String url, String usernameHiring, String passwordHiring,
		...
		test = extent.createTest(
				"Starting End To End Flow NewOrderHiring + Employee Creation + Claim Order + Approve Claim + Assign Employee + Timesheet Creation");
		...
		Thread.sleep(2000);
		deny.clickDenyButton();
		Thread.sleep(2000);
		deny.clickmodalDenyButton();
		Thread.sleep(2000);
		deny.closeMessagePopup();
		Thread.sleep(2000);
		Assert.assertEquals(deny.verifyDeniedStatus().contains("Denied"), true, "Message is not displayed");
		Thread.sleep(2000);
